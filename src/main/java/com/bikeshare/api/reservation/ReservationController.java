...
public class ReservationController {
    ...
    @Operation(summary = "Listar reservas", description = "Permite filtrar por renterId, ownerId y status")
    @GetMapping
    public List<ReservationDTO> list(
            @Parameter(description = "ID del arrendatario (renter)") @RequestParam(required = false) Long renterId,
            @Parameter(description = "ID del propietario (owner)") @RequestParam(required = false) Long ownerId,
            @Parameter(description = "Estado de la reserva") @RequestParam(required = false) ReservationStatus status
    ) {
        return service.list(renterId, ownerId, status).stream()
                .map(ReservationDTO::from)
                .toList();
    }

    @Operation(summary = "Actualizar el estado de una reserva (para propietarios)")
